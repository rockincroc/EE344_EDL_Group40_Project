...
void loop(void) 
{
    ...
    EEPROM.write(lastwrittento, (int)(temp*6));
    EEPROM.commit();
    ...
    { 
        ...
        for (int i = 0; i<previous; i++){
          //client.println("Time:<input type=\"text\" id=\"currentTime\"><br><br><script>var today = new Date();var time = today.getHours() + \":\" + ((today.getMinutes() - (i*10)%60)%60) + \":\" + ((today.getSeconds()-i*10)%60) ;document.getElementById(\"currentTime\").value = time;</script>");
          //client.println("Time:<input type=\"text\" id=\"currentTime\"><br><br><script>var today = new Date();var time = today.getHours() + \":\" + today.getMinutes() + \":\" + today.getSeconds() ;document.getElementById(\"currentTime\").value = time;</script>");
          //client.println("<p style=\"color:red\">Temperature in degrees Celsius before ");
          client.print("<br><br><script>var today = new Date(Date.now() - ");
          client.print(i*10000);
          client.print("); document.write(today);</script>,");
          //client.println(i*10);
          //client.println("seconds is ");
          int readval = (EEmax + lastwrittento - i - 2)%EEmax;
          //client.print(", ");
          client.print(EEPROM.read(readval)/6.0);
          client.println("*C</p>\n");
        }
